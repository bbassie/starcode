...
Next to the setting chips the composer has a gauge: how much of the model's context window this conversation takes up. Claude Code states the size of every request on the assistant message it produced and names the window in the turn's result; Codex sends both in `thread/tokenUsage/updated`. Either way the bar climbs while the turn runs, not only when it ends. It turns amber at 75% and red at 90%, near where the agents start compacting on their own.

Clicking the gauge opens the model picker, since a model with more room is the way out of a full window. Every row there shows its size (200K, 1M), so the trade is visible before it is made. Picking another model keeps the token count and forgets the old window until the next turn reports the new one, so the percentage moves as soon as the model does.
